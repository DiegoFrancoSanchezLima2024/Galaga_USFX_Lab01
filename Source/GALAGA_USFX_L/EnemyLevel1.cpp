...
AEnemyLevel1::AEnemyLevel1()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> MeshAsset(TEXT("StaticMesh'/Game/TwinStick/Meshes/EnemyLevel1.EnemyLevel1'"));
	if (MeshAsset.Succeeded())
	{
		Nave_Mesh->SetStaticMesh(MeshAsset.Object);

		// Modificar la escala del componente de malla
		FVector NewScale(1.f, 1.f, 1.f); // Escala modificada
...
void AEnemyLevel1::Damage(float Damage)
{
		Life -= Damage;
}
...
void AEnemyLevel1::ComponenteDeDestruccion()
{
	if (Explosion_Nave)
